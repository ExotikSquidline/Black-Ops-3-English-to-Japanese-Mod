        ...
        public static GameDataTable.Asset ConvertAssetBufferToGDTAsset(byte[] assetBuffer, Tuple<string, int, int>[] properties, HydraInstance instance, Func<GameDataTable.Asset, byte[], int, int, HydraInstance, object> extendedDataHandler = null)
        {
            ...
            foreach (var property in properties)
            {
                switch (property.Item3)
                {
                    ...
                    case 1:
                        {
                            asset[property.Item1] = Encoding.UTF8.GetString(assetBuffer, property.Item2, 1024).TrimEnd('\0');
                            break;
                        }
                    // Inline Character Array
                    case 2:
                        {
                            asset[property.Item1] = Encoding.UTF8.GetString(assetBuffer, property.Item2, 64).TrimEnd('\0');
                            break;
                        }
                    // Inline Character Array
                    case 3:
                        {
                            asset[property.Item1] = Encoding.UTF8.GetString(assetBuffer, property.Item2, 256).TrimEnd('\0');
                            break;
                        }
                    // 32Bit Ints
                    case 4:
