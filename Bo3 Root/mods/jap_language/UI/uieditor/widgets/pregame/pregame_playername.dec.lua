...
CoD.Pregame_PlayerName.new = function ( menu, controller )
	...
	Name:linkToElementModel( self, "gamertag", true, function ( modelRef )
		local gamertag = Engine.GetModelValue( modelRef )
		if gamertag then
	...
	ListHeader:linkToElementModel( self, "gamertag", true, function ( modelRef )
		local gamertag = Engine.GetModelValue( modelRef )
		if gamertag then
	...
	btnDisplayTextStroke:setLeftRight( true, false, 5, 220 )
	...
	btnDisplayTextStroke:linkToElementModel( self, "gamertag", true, function ( modelRef )
		local gamertag = Engine.GetModelValue( modelRef )
		if gamertag then
	...
	self.clipsPerState = {
		...
		IsSelf = {
			DefaultClip = function ()
				...
				self.clipFinished( ListHeader, {} )
				btnDisplayTextStroke:completeAnimation()
				self.btnDisplayTextStroke:setRGB( 0.15, 0.55, 1 )
				self.btnDisplayTextStroke:setAlpha( 1 )
	...
	self:linkToElementModel( self, "xuid", true, function ( modelRef )
